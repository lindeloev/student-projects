# -*- coding: utf-8 -*-
"""
N-way CMC for a study on individual difference in CMC. In particular, this
study examines CMCs between up to four visual features (shape, complexity, colour, 
and lightnesss), and up to two auditory features (pitch and timbre).
...
TO DO:
 * Fix beep duration in CMC rating
"""
...
DIALOGUE = {
    ...
    'refresh_rate': [60, 100],
    'experimenter': ['person1', 'person2', 'person3', 'person4', 'person5']
}
...
# Conditions
TEST_REPETITIONS = 2  # Repetition of each trial type
...
TEST_FEATURES = ['shape', 'complexity', 'lightness', 'timbre', 'pitch']  # Test these
RATE_FEATURES = ['shape', 'complexity', 'hue', 'lightness', 'timbre', 'pitch']  # Rate combinations of these

# Responses
RESPONSE_KEYS = ['left', 'right']
RESPONSE_TEXT = {'left': u'\u2190', 'right': u'\u2192'}  # Alternatives: ← →, ⬅ ➡, ◀ ▶, 'LEFT', 'RIGHT'
CATCH_TEXT = {'up': u'\u2191', 'down': u'\u2193'}
#RESPONSE_TEXT = {'left': u'\u2190', 'right': u'\u27A1'}
CATCH_KEYS = ['up', 'down']
QUIT_KEYS = ['escape']
CONTINUE_KEYS = ['enter', 'return']
...
if DIALOGUE['language'] != 'en':
    language = gettext.translation('CMC_5way', localedir='locale', languages=[DIALOGUE['language']])
    if ppc.python3:
        language.install()
    else:
...
texts = {
    ...
    'bottom_reminder': _(u' The reminder you see at the bottom will be present throughout the task. Press %(left)s or %(right)s a few times to see/hear more examples.\n\nRemember, once in a while you will see an arrow or hear a sliding chirp. Use the up-arrow and down-arrow to respond to those.') % RESPONSE_TEXT
}
texts.update({
...
def make_trials(task, phase='exp'):
    ...
    # All possible combinations of test features - and add repetitions.
    conjunctions = itertools.product(
        features['shape'] if 'shape' in TEST_FEATURES else ['neutral'], 
        features['complexity'] if 'complexity' in TEST_FEATURES else ['neutral'],
        features['hue'] if 'hue' in TEST_FEATURES else ['neutral'],
        features['lightness'] if 'lightness' in TEST_FEATURES else ['neutral'],
        features['timbre'] if 'timbre' in TEST_FEATURES else ['neutral'],
        features['pitch'] if 'pitch' in TEST_FEATURES else ['neutral'],
        range(TEST_REPETITIONS))
    
    # Loop through them
    for shape, complexity, hue, lightness, timbre, pitch, _ in conjunctions:  # _ for repetition number
        trial = OrderedDict({
            # Figure properties
            'shape': shape,
...
def demo_figure(task):
    """Show which key 'belongs' to which figure attribute."""
    # Loop until a key is pressed
    trials = make_trials(task, phase='demo')
    while True:
...
def rate_CMC(feature1, feature2):
    ...
    trials = make_trials('shape', phase='rate')
    trial = random.choice(trials)  # Use one base stimulus
    
    # Start by "neurtralizing" all non-targeted features
    non_features = set(features.keys()) - set((feature1, feature2))
    for feature in non_features:
        trial[feature] = 'neutral'
            ...
            for group in groups:
                for example in examples:
                    ...
                    if mouse.isPressedIn(figure_fill):
                        ...
                        while mouse.isPressedIn(figure):
                            pass
...
def run_block(task, phase='exp'):
    ...
    for no_task, trial in enumerate(trials):
        ...
        while True:
            ...
            if DIALOGUE['simulation'] == 'no':
                fixation.draw()
                key, rt = event.waitKeys(keyList=RESPONSE_KEYS + CATCH_KEYS + QUIT_KEYS, timeStamped=clock)[0]
            else:
...
if DIALOGUE['section'] == 'rate_CMC':
    ...
    if DIALOGUE['demo_and_practice'] == 'yes':
        ...
        run_block('shape', phase='practice_catch')
    ...
    for superblock in range(N_superblocks):  # Assuming that all have equal length        
        ...
        for i, task in enumerate(TEST_FEATURES):
            ...
            if DIALOGUE['demo_and_practice'] == 'yes':
                ...
                run_block(task, phase='practice_exp')

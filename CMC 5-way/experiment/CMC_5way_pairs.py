...
DIALOGUE = {
    ...
    'refresh_rate': [60, 100],
    'experimenter': ['person1', 'person2', 'person3', 'other']
}
...
TEST_FEATURES = [
    (0, 'lightness', 'pitch'),  # i, task, inducer
    (0, 'pitch', 'lightness'),
    (0, 'shape', 'lightness'),
    (0, 'lightness', 'shape')
]
...
if False:
    ...
    # Two super-blocks
    N_superblocks = 1  #len(key_feature_maps['shape'])  # Number of response orders
    for superblock in range(N_superblocks):  # Assuming that all have equal length
        # Run tasks in random order for each super-
        for i, task, inducer in TEST_FEATURES:
        #for i, task in enumerate(TEST_FEATURES):
            ...
            if DIALOGUE['demo_and_practice'] == 'yes':
                ...
                if task in ('timbre', 'pitch'):
                    demo_beep(task, inducer)
                else:
                    demo_figure(task, inducer)

                # Practice it
                show_instruction(texts['practice_exp'])
                run_block(task, inducer, phase='practice_exp')
